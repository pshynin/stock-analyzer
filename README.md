...
```bash
stock-analyzer$ sam build --use-container
```
...
```bash
stock-analyzer$ sam local invoke HelloWorldFunction --event events/event.json
```
...
```bash
stock-analyzer$ sam logs -n HelloWorldFunction --stack-name stock-analyzer --tail
```
...
```bash
stock-analyzer$ pip install pytest pytest-mock --user
stock-analyzer$ python -m pytest tests/ -v
```
...
```bash
aws cloudformation delete-stack --stack-name stock-analyzer
```
